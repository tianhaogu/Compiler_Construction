...
Scope *ClassDecl::PrepareScope()
{
    ...
    for (int i = 0; i < implements->NumElements(); i++) {
        NamedType *in = implements->Nth(i);
	if (in->IsInterface())
	    convImp->Append(dynamic_cast<InterfaceDecl*>(in->FindDecl(in->GetId())));
    }
    for (int i = 0; i < members->NumElements(); i++) {
...
void FnDecl::Check() {
	...
	// only check the types of formals in interface
	for (int i = 0; i < formals->NumElements(); ++i)
	    formals->Nth(i)->GetDeclaredType()->Check();
    }
}
